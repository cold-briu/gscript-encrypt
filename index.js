const config = {
  ...
  prompt: {
    ...
    cancelKey: "cancel",
    error: "Please Enter A Key!  Encryption Aborted!"
  }
}
...
var wordbyte0 = function (w) { return Math.floor(Math.floor(Math.floor(w / 256) / 256) / 256) % 256 };
var wordbyte1 = function (w) { return Math.floor(Math.floor(w / 256) / 256) % 256 };
var wordbyte2 = function (w) { return Math.floor(w / 256) % 256 };
var wordbyte3 = function (w) { return w % 256 };

var xl_par = 0x00000000;
var xr_par = 0x00000000;
...
function encrypt(t, k) {
  ...
  wordbyte0 = function (w) { return Math.floor(Math.floor(Math.floor(w / 256) / 256) / 256) % 256 };
  wordbyte1 = function (w) { return Math.floor(Math.floor(w / 256) / 256) % 256 };
  wordbyte2 = function (w) { return Math.floor(w / 256) % 256 };
  wordbyte3 = function (w) { return w % 256 };

  k = k + '';
  Blowfish(k);

  for (var f = 0; f < t.length % 8; f++) {
    t += " ";
  }
  ...
  for (var f = 0; f < t.length; f += 8) {
    ...
    enc += wordescape(xl_par) + wordescape(xr_par);
...
function decrypt(t, k) {
  ...
  for (var f = 0; f < t.length; f += 16) {
    var l = unescape(t.substr(f, 8));
    var r = unescape(t.substr(f + 8, 8));
    ...
    dec += wordescape(xl_par) + wordescape(xr_par);
  }
...
function Blowfish(k) {
  ...
  key = (k.length > 56) ? k.substr(0, 56) : k;

  var j = 0;

  for (var i = 0; i < 18; ++i) {
    var d = ((key.charCodeAt(j % key.length) * 256 + key.charCodeAt((j + 1) % key.length)) * 256 + key.charCodeAt((j + 2) % key.length)) * 256 + key.charCodeAt((j + 3) % key.length);
...
function wordescape(w) {
  var r = '';
  var m = new Array(wordbyte3(w), wordbyte2(w), wordbyte1(w), wordbyte0(w));
  for (var i = 3; i >= 0; i--) {
...
function round(a, b, n) {
  ...
  return (xor(a, xor(((xor((bf_S0[wordbyte0(b)] + bf_S1[wordbyte1(b)]), bf_S2[wordbyte2(b)])) +
    bf_S3[wordbyte3(b)]), bf_P[n])));
};
